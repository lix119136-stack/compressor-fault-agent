import streamlit as st  # <--- 【核心修复】确保第一行精准导入，彻底消灭 NameError
import pandas as pd
import numpy as np
import torch
...
def load_diagnostic_resources():
    """使用 streamlit 缓存机制，确保神经网络和配置在云端只加载一次"""
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    label_encoder = joblib.load("label_encoder.pkl")
    scaler = joblib.load("data_scaler.pkl")

    # 动态匹配 11 分类的高级自适应池化网络
...
# 2. 网页布局设计
# =====================================================================
st.title("⚙️ 往复压缩机故障诊断智能体系统 (Web 3.0 完全体)")
st.markdown("---")
...
with col2:
    ...
    if uploaded_file is not None:
        # ---- 阶段 1：静态组件一次性渲染完毕（不与大模型并行，防止前端DOM树崩溃） ----
        with st.spinner("⚡ 神经网络正在提取波形模式..."):
            try:
                # 精准只提取前两列容积与压力，自动剔除可能存在的干扰坏点
